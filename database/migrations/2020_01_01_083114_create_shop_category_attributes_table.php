...
class CreateShopCategoryAttributesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('shop_category_attributes', function (Blueprint $table) {
            ...
            $table->string('name')->default('')->limit(20)->nullable(false)->comment('名称');
            $table->string('type')->default('')->limit(6)->nullable(false)->comment('属性类型');
            $table->string('values')->default('')->limit(500)->comment('属性值列表');
