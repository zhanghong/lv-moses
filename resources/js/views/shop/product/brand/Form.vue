  ...
  <div class="app-container">
    <el-form ref="postForm" :model="postForm" :rules="rules" label-width="150px">
      <el-form-item :label="fields.name" prop="name">
        <el-input v-model.trim="postForm.name" placeholder="请输入品牌名称" />
      </el-form-item>

      <el-form-item :label="fields.description" prop="description">
        <el-input v-model.trim="postForm.description" :row="4" type="textarea" placeholder="请输入品牌介绍" />
      </el-form-item>
...
const modelResource = new Resource('shop/product/brands');
...
export default {
  ...
  data() {
    ...
    return {
      ...
      fields: {
        name: this.$t('product_brand.name'),
        description: this.$t('product_brand.description'),
        order: this.$t('base_fields.order'),
      },
      rules: {
        name: [
          { required: true, message: '品牌名称 不能为空', trigger: 'blur' },
          { min: 2, max: 20, message: '品牌名称 长度在 2 到 20 个字符', trigger: 'blur' },
          { validator: validateNameUnique, trigger: 'blur' },
        ],
        description: [
          { max: 100, message: '品牌简介 长度不能超过 100 个字符', trigger: 'blur' },
        ],
        order: [
  ...
  methods: {
    ...
    goToList() {
      ...
      this.$router.push({ name: 'shopProductBrandList' });
    },
  },
};
</script>
